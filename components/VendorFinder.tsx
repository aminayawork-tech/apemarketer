    ...
    <div className="mt-4 space-y-3">
      ...
      {vendors.length === 0 ? (
        <p className="text-[#A09590] text-xs text-center py-3">
          No vendors with 4.5★+ found within 6 miles.
        </p>
      ) : (
            ...
            <div key={i} className="bg-[#1a1a1a] rounded-lg border border-[#2a2a2a] hover:border-[#E05C0A]/30 transition-colors overflow-hidden">
              <div className="px-3 py-3">
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  {v.openNow !== null && (
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded flex-shrink-0 ${v.openNow ? "bg-green-900/40 text-green-400" : "bg-red-900/30 text-red-400"}`}>
                      {v.openNow ? "Open" : "Closed"}
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-1.5 mb-1.5">
                  <span className="text-[#E05C0A] text-xs">{"★".repeat(Math.floor(v.rating))}</span>
                  <span className="text-[#F2EDE4] text-xs font-bold">{v.rating}</span>
                  <span className="text-[#6B5F57] text-xs">({v.ratingsTotal})</span>
                </div>
